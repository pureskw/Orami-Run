...
milestoneImage.src = "images.jpeg";
...
function loop(now) {
  ...
  if (!milestoneActive) {
    ...
    milestoneElapsed = (now - milestoneStart) / 1000;
  }
  render();
...
function renderMilestone() {
  ...
  const imageFade = 4;
  const t = milestoneElapsed;

  ctx.fillStyle = "rgba(10, 60, 45, 0.65)";
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  const messageAlpha = Math.max(0, 1 - t / messageFade);
  if (messageAlpha > 0) {
  ...
  const imageAlpha = Math.min(1, Math.max(0, (t - messageFade) / imageFade));
  if (imageAlpha > 0 && milestoneImage.complete) {
    const maxW = canvas.width * 0.7;
    const maxH = canvas.height * 0.7;
    const ratio = Math.min(maxW / milestoneImage.width, maxH / milestoneImage.height);
    const drawW = milestoneImage.width * ratio;
    const drawH = milestoneImage.height * ratio;
    const x = (canvas.width - drawW) / 2;
    const y = (canvas.height - drawH) / 2;
    ctx.globalAlpha = imageAlpha;
    ctx.drawImage(milestoneImage, x, y, drawW, drawH);
    ctx.globalAlpha = 1;
  }
}
...
window.addEventListener("pointerdown", (e) => {
  if (e.target && e.target.closest && e.target.closest("#restart")) return;
  jump();
});
